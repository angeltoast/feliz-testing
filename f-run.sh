...
function mirror_list {  # Use rankmirrors (script in /usr/bin/ from Arch) to generate fast mirror list !!! rankmirrors no longer exists
  ...
  if [ -f mirrors.list ] && [ "$(wc mirrors.list)" -gt 1 ]; then  # If user has entered a manual list of >0 mirrors
    ...
    echo mirrors.list | grep '^Server' >> /etc/pacman.d/mirrorlist
    ...
    echo usemirrors.list | grep '^Server' >> /etc/pacman.d/mirrorlist
  fi
}

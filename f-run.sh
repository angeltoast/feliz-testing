...
function mirror_list {  # User has selected one or more countries with Arch Linux mirrors
  ...
  if [ -f mirrors.list ] && [ "$(wc mirrors.list)" -gt 1 ]; then  # If user has entered a manual list of >0 mirrors
    ...
    Date=$(date)
    echo -e "# Mirrors /etc/pacman.d/mirrorlist \n# $Date \n# Generated by ${user_name}\n#" > /etc/pacman.d/mirrorlist
    # copy mirrors in mirrors.list to /etc/pacman.d/mirrorlist
    meers=$(cat mirrors.list) # Copy list into variable
    for i in $meers; do       # Add each one to /etc/pacman.d/mirrorlist
      echo $i | grep '^Server' >> /etc/pacman.d/mirrorlist
    done
    
  elif [ ${#CountryLong[@]} -eq 0 ]; then  # If no mirrors were chosen by user ...
    install_message "Ranking mirrors - please wait ..."
